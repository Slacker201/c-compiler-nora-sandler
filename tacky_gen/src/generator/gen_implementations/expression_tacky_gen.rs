use slk_c_core::{parser_core::parser_ir::{Expression, ExpressionKind, UnaryOp}, tacky_gen::{TackyInstruction, Value, VariableId}};
    ...
    fn generate_tacky(&self) -> (VariableId, Vec<TackyInstruction>) {
        ...
        match self.kind() {
            ExpressionKind::Constant(constant) => {
                ...
                instructions.push(
                    TackyInstruction::LoadVariable(variable_id, Value::from_ast_constant(constant))
                );

                return (variable_id, instructions);
            },
            ExpressionKind::Unary(unary_op, expression) => {
                match unary_op {
                    UnaryOp::Negate => {
